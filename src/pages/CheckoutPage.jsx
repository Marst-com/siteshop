...
export default function CheckoutPage() {
  ...
  const saveOrder = async (paymentKey = null, method = '무료') => {
    await addDoc(collection(db, 'orders'), {
      ...
      items: cart.map(i => ({ productId: i.id, name: i.name, price: i.price, quantity: i.quantity, sellerId: i.sellerId || '' })),
      total,
    ...
    if (earnedPoints > 0) {
      ...
      await updateDoc(userRef, { points: (snap.data()?.points || 0) + earnedPoints });
    }

    // salesCount 증가
    await Promise.all(cart.map(i =>
      updateDoc(doc(db, 'products', i.id), { salesCount: increment(i.quantity) })
    ));
